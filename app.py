...
st.set_page_config(page_title="Personel Bilgi Sistemi", layout="wide")
...
with st.form("akilli_form", clear_on_submit=True):
    # --- GÖREV YERİ SEÇİMİ (İl / İlçe Mekanizması) ---
    st.subheader("📍 Görev Yeri Bilgileri")
    c_il1, c_il2 = st.columns(2)
    
    with c_il1:
        secilen_il = st.selectbox("Görev Yeri (İl) *", options=list(sehirler.keys()))
    with c_il2:
        # İl seçildiğinde ilgili ilçeleri getiriyoruz
    ...
    with col_v1:
        gidis_v = st.radio("Gidiş Vasıtası *", ["Uçak", "Otobüs"], horizontal=True)
    with col_v2:
        donus_v = st.radio("Dönüş Vasıtası *", ["Uçak", "Otobüs"], horizontal=True)
    ...
    with col3:
        derece = st.text_input("Derece")
        kademe = st.text_input("Kademe")
        iban = st.text_input("IBAN (24 hane) *")

    st.divider()

    # --- BİLET DETAYLARI VE DOSYA YÜKLEME ---
    st.subheader("🎫 Bilet Bilgileri")
    g_c1, g_c2 = st.columns(2)
    with g_c1:
        g_tutar = st.number_input(f"{gidis_v} Gidiş Tutarı (₺)", min_value=0)
        st.file_uploader("Gidiş Bileti Görseli", type=['pdf', 'jpg', 'png'])
    with g_c2:
        d_tutar = st.number_input(f"{donus_v} Dönüş Tutarı (₺)", min_value=0)
        st.file_uploader("Dönüş Bileti Görseli", type=['pdf', 'jpg', 'png'])
    ...
    if submit:
        if not (tc and ad and soyad and iban):
            st.warning("Lütfen yıldızlı (*) alanları boş bırakmayın!")
        else:
            try:
                # Notion'a kayıt (Sütun isimlerinin birebir aynı olduğundan emin ol)
                notion.pages.create(
                    ...
                    properties={
                        ...
                        "Gidiş Vasıtası": {"select": {"name": gidis_v}},
                        "Dönüş Vasıtası": {"select": {"name": donus_v}},
                        "Gidiş Bileti Tutarı (₺)": {"number": g_tutar},
                        "Dönüş Bileti Tutarı (₺)": {"number": d_tutar}
                    }
                )
                st.success(f"Sayın {ad} {soyad}, verileriniz Notion'a kaydedildi!")
                st.balloons()
